...
def train(model, dataset, train_type, index, seed, num_class, decay, bs, value, epoch, lr, min_loss):
    ...
    for i in range(epoch):
        ...
        print('\nEpoch %d/%d \n' % (i + 1, epoch) + '-' * 60, file=open(f"/content/drive/My Drive/Project_SS24/MedAugment/recording/{train_type}/{con_str}log.txt", "a"))
        ...
        t_c // 60, t_c % 60, t_loss, t_dice, v_loss, v_dice, v_iou, v_pa), file=open(f"/content/drive/My Drive/Project_SS24/MedAugment/recording/{train_type}/{con_str}log.txt", "a"))
        ...
        for step, (name, te_x, te_y) in enumerate(test_l):
            ...
            if v_loss < min_loss:
                for i in range(output.size(0)):
                    pre_path = f"/content/drive/My Drive/Project_SS24/MedAugment/recording/{train_type}/{con_str}pre"
        ...
        print('Test done in %d m %d s \nTest loss: %.3f, dice: %.3f, iou: %.3f, pa: %.3f' % (t_c // 60, t_c % 60,
              te_loss, te_dice, te_iou, te_pa), file=open(f"/content/drive/My Drive/Project_SS24/MedAugment/recording/{train_type}/{con_str}log.txt", "a"))

        if v_loss < min_loss:
            stop_index = 0
            min_loss = v_loss
            torch.save(model, f"/content/drive/My Drive/Project_SS24/MedAugment/recording/{train_type}/{con_str}model.pkl")
            print("Model saved", file=open(f"/content/drive/My Drive/Project_SS24/MedAugment/recording/{train_type}/{con_str}log.txt", "a"))
        else:
            stop_index += 1

        if stop_index == 8: # increased from 8 to 12
            print("Early stopping triggered", file=open(f"/content/drive/My Drive/Project_SS24/MedAugment/recording/{train_type}/{con_str}log.txt", "a"))
            break
...
def segmentation():
    ...
    group.add_argument('--model', help='Choose your own model', choices=['UNetPP', 'FPN', 'DeepLab'], default='UNetPP')
