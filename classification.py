...
sys.path.append('/content/drive/MyDrive/Project_SS24/MedAugment/utils')
...
def train(model, dataset, train_type, index, seed, num_class, decay, bs, value, epoch, lr, min_loss):
    ...
    # Open log file once for the entire training session
    log_file_path = f"/content/drive/MyDrive/Project_SS24/MedAugment/recording/{train_type}/{con_str}log.txt"
    with open(log_file_path, "w") as log_file:
        print('Batch size: %d\nLearning rate: %s\nNumber of epochs: %d' % (bs, lr, epoch), file=log_file)
        log_file.flush()  # Ensure initial log is written

        for i in range(epoch):
            print("Epoch:", i)
            t_loss, v_loss, t_loss_b, v_loss_b = 0, 0, 0, 0
            t_tle, t_ple, v_tle, v_ple = [], [], [], []  # predicted, true; label; epoch
            print('\nEpoch %d/%d \n' % (i + 1, epoch) + '-' * 60, file=log_file)
            log_file.flush()  # Flush after each epoch start
            ...
            for step, (t_x, t_y) in enumerate(train_l):
                ...
                if step % 10 == 0:
                    print(f'Step {step}, Training loss: {loss.item():.3f}', file=log_file)
                    print(f'Step {step}, Training loss: {loss.item():.3f}')  # Also print to console
            ...
            print('Train and validation done in %d m %d s \nTrain loss: %.3f, acc: %.3f; Val loss: %.3f, acc: %.3f, '
                  'npv: %.3f, ppv: %.3f, sen: %.3f, spe: %.3f, fos: %.3f' % 
                  (t_c // 60, t_c % 60, t_loss, t_acc, v_loss, v_acc, v_npv, v_ppv, v_sen, v_spe, v_fos), 
                  file=log_file)
            log_file.flush()  # Ensure log is updated after each epoch
            ...
            print('Test done in %d m %d s \nTest loss: %.3f, acc: %.3f, npv: %.3f, ppv: %.3f, sen: %.3f, spe: %.3f, '
                  'fos: %.3f' % 
                  (t_c // 60, t_c % 60, te_loss, te_acc, tev_npv, te_ppv, te_sen, te_spe, te_fos),
                  file=log_file)
            log_file.flush()  # Ensure log is updated after testing

            if v_loss < min_loss:
                stop_index = 0
                min_loss = v_loss
                torch.save(model, f"/content/drive/MyDrive/Project_SS24/MedAugment/recording/{train_type}/{con_str}model.pkl")
                print("Model saved", file=log_file)
                log_file.flush()  # Ensure log is updated after saving the model
            else:
                stop_index += 1

            if stop_index == 8:
                print("Early stopping triggered", file=log_file)
                log_file.flush()  # Ensure log is updated before breaking
                break
...
def classification():
    ...
    group.add_argument('--index', help='Index for method of run', required=True, choices=[1, 2, 3, 4, 5], type=int, metavar='INT', default=5)
